...
import {
  ...
  useNotificationControllerDeleteNotification,
        ...
        return {
          ...
          pages: old.pages.map((page: any) => ({
            ...
            data: {
              ...page.data,
              data: (page.data?.data || []).map((n: any) =>
                n.id === notification.id ? { ...n, is_read: isRead } : n,
              ),
            },
          })),
        };
      ...
      queryClient.setQueriesData(
        ...
        (old: any) => {
          if (old?.data?.unread_count > 0) {
            return {
              ...old,
              data: { ...old.data, unread_count: old.data.unread_count - 1 },
            };
          }
          return old;
        },
    ...
    if (target_type === 'POST') {
      try {
        ...
        if (error.response?.status === 404) {
          ...
          toast({
            title: 'Không thể tải nội dung, vui lòng kiểm tra kết nối mạng',
          });
    ...
    >
      ...
      <Avatar className="w-11 h-11 border bg-muted shrink-0">
        <AvatarImage
          src={
            primaryActor?.avatar
              ? primaryActor.avatar.startsWith('http')
                ? primaryActor.avatar
                : `${import.meta.env.VITE_MEDIA_URL}/${primaryActor.avatar}`
              : '/default-avatar.png'
          }
          alt={primaryActor?.username || 'User'}
          className="object-cover"
        />
        <AvatarFallback>
          {primaryActor?.username?.[0]?.toUpperCase() || 'U'}
        </AvatarFallback>
      </Avatar>
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center text-[14px]">
        ...
        <span className="text-muted-foreground text-[12px] mt-0.5 leading-none">
          {timeAgo}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 shrink-0 ml-1">
        {notification.notification_type === 'FOLLOW' && (
          <Button
            size="sm"
            variant="secondary"
            className="h-8 text-xs font-semibold px-3 mr-1"
